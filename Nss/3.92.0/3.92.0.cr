class Target < ISM::Software
   ...
   def build
    ...
    def prepareInstallation
        ...
        copyFile(Dir["#{workDirectoryPath(false)}/dist/Linux*/bin/certutil"],"#{builtSoftwareDirectoryPath(false)}#{Ism.settings.rootPath}usr/bin/certutil")

        copyFile(Dir["#{workDirectoryPath(false)}/dist/Linux*/bin/nss-config"],"#{builtSoftwareDirectoryPath(false)}#{Ism.settings.rootPath}usr/bin/nss-config")

        copyFile(Dir["#{workDirectoryPath(false)}/dist/Linux*/bin/pk12util"],"#{builtSoftwareDirectoryPath(false)}#{Ism.settings.rootPath}usr/bin/pk12util")
